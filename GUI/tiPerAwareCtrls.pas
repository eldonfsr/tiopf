...
begin
  ...
  lsNone    : begin
                ...
                FWinControl.SetBounds(0,0, Self.Width, Self.Height);
              end ;
  lsTop     : begin
                FWinControl.Top    := FLabel.Top + FLabel.Height;
                FWinControl.Left   := 0 ;
                FWinControl.Height := Height - FWinControl.Top;
                FWinControl.Width  := Width  - FWinControl.Left;
              end ;
  lsLeft    : begin
                FWinControl.Top    := 0 ;
                FWinControl.Left   := LabelWidth;
                FWinControl.Height := Height - FWinControl.Top;
                if Width > LabelWidth then
                begin
                  {$IFDEF FPC}
                  if FWinControl is TDateEdit then
                    FWinControl.Width  := Width  - FWinControl.Left - TDateEdit(FWinControl).ButtonWidth
                  else
                  {$ENDIF}
                  FWinControl.Width  := Width  - FWinControl.Left;
                end;
...
begin
  ...
  if Assigned(FLabel) then PositionLabel;
  if Assigned(FWinControl) then PositionWinControl;
end;
{$ENDIF}
...
begin
  ...
  TDateTimePicker(FWinControl).Date :=
    Trunc(Int64(GetPropValue(FData, FsFieldName, True)));
  {$ENDIF}
  SetOnChangeActive(true);
end;
...
//  FScrollBContainer.BorderStyle := bsSingle;
  ...
  FScrollBox.Parent := FWinControl;  //FScrollBContainer;
  FScrollBox.Top   := 0;
  FScrollBox.Left  := 0;
...
begin
  inherited;
  if Assigned(FLabel) and (FLabel.Visible or (VisibleButtons <> [])) then
  begin
    //FScrollBox.Align := alNone;
    FScrollBox.Height := FWinControl.Height - FScrollBox.Top;
    FScrollBox.Width := FWinControl.Width ;
  end
  else
  begin
    //FScrollBox.Align := alClient;
    if Assigned(FScrollBox) then
      FScrollBox.SetBounds(0, 0,  FScrollBox.Parent.Width, FScrollBox.Parent.Height);
  end;
end;
